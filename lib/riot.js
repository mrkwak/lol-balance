...
function parseRiotApiError(err) {
  ...
  console.error('Riot API error:', status, err.response?.data, 'Key prefix:', (process.env.RIOT_API_KEY || '').substring(0, 15));
  if (status === 404) return '소환사를 찾을 수 없습니다';
  if (status === 401 || status === 403) return `API 키 오류 (${status})`;
