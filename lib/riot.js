...
function parseRiotApiError(err) {
  ...
  const keyInfo = process.env.RIOT_API_KEY ? `set(${process.env.RIOT_API_KEY.substring(0,10)}...)` : 'NOT_SET';
  if (status === 404) return '소환사를 찾을 수 없습니다';
  if (status === 401 || status === 403) return `API 키 오류 (${status}) key=${keyInfo} detail=${detail}`;
  if (status === 429) return 'API 요청 제한 초과 (잠시 후 재시도)';
  return `${err.message || '알 수 없는 오류'} (status=${status}) key=${keyInfo}`;
}
